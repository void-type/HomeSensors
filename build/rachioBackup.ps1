...
foreach ($device in $devices) {
  ...
  while ($startTime.ToUnixTimeMilliseconds() -ge $device.createDate) {
    $response = Invoke-RestMethod -Uri "https://api.rach.io/1/public/device/$($device.id)/event?startTime=$($startTime.ToUnixTimeMilliseconds())&endTime=$($endTime.ToUnixTimeMilliseconds())" -Method GET -Headers $headers
    $allEvents += $response
